...
export const ContentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [pageContent, setPageContent] = useState<PageContentState>(() => {
    try {
      const saved = localStorage.getItem('wrk_site_pages_v16'); // Changed key to avoid loading old structure
  ...
  useEffect(() => {
    localStorage.setItem('wrk_site_pages_v16', JSON.stringify(pageContent));
  }, [pageContent]);
