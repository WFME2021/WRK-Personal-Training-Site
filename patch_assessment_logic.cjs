...
const logicContent = `import { assessmentData } from '../data/assessmentData.ts';
...
export function calculateArchetype(answers: Answers) {
  ...
  if (answers['q6_flags'] && answers['q6_flags'] !== 'none') {
    ...
    if (answers['q6_flags'] === 'manageable') {
      block = modificationBlocks.manageable(subAnswer);
    } else if (answers['q6_flags'] === 'limits') {
      block = modificationBlocks.limits(subAnswer);
...
fs.writeFileSync('services/assessmentLogic.ts', logicContent);
console.log('Updated assessmentLogic.ts');
